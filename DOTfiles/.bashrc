...
echo -ne "${lightblue}Operating System: \t${cyan}" `lsb_release -a`;
echo ""
...
__oc_ps1()
{
    ...
    if [ -n "$CONTEXT" ]; then
        echo -ne "\e[1;30m(${lightgreen}K8Context:${lightgreen}${CONTEXT}\e[1;30m)"
    fi
}
parse_git_branch() {
    GIT_CONTEXT=$(git branch 2> /dev/null | sed -e '/^[^*]/d' -e 's/* \(.*\)/ (\1)/')

     if [ -n "$GIT_CONTEXT" ]; then
        echo -ne "${lightgreen}${GIT_CONTEXT}${darkgray}" | xargs
    fi
}

export PS1='\n$(__oc_ps1)$(parse_git_branch)\e[1;30m(\e[1;32m`date +%T`\e[1;30m)\n\[\033[0;32m\]└\[\033[0m\033[0;32m\]\e[1;31m \u@\h\e[1;35m \W\e[0;37m \$\[\033[0m\033[0;32m\] ▶\[\033[0m\] ' 
...
alias tc="tmux new -s newsession"
